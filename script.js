...
document.getElementById('orderButton').addEventListener('click', function() {
    ...
    Nr: <span>${orderNrCurrent}</span>
    Order: <span>${document.getElementById('selectedGuest').value}</span>
        bestellt: <span>${document.getElementById('selectedDrink').value}</span>
       
        Time: <span id="orderTime${orderNrCurrent}">${formatTime(runningOrderTime)}</span>
        <button class="submitOne" style="background-color: red;"></button>
    `;
